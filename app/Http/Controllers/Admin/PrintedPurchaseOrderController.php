...
use App\Http\Requests\Admin\PoRequest;
use Illuminate\Support\Facades\Storage;
use App\Models\FoldMaster;
use App\Models\YarnMaster;
use App\Models\Category;
use App\Models\PrintedPurchaseOrder;
use File;

class PrintedPurchaseOrderController extends Controller
{
    public function index(Request $request)
    {
        //$purchaseOrders = DesignCard::with(['salesRepDetail','customerDetail'])->whereIn('type',['printed','print_po']);
        
         $purchaseOrders = PrintedPurchaseOrder::with('designCard')->where('design_id', '<>',NULL )->where('design_id', '<>','' );
         
         
        
        $purchaseOrders = $purchaseOrders->paginate(config("motorTraders.paginate.perPage"));
        
      //  dd($purchaseOrders);
        
        return view("printedpurchaseorder.index", compact("purchaseOrders"));
    }


    public function create()
    {   
         $Po = PrintedPurchaseOrder::firstOrCreate(['design_id'=>'']);
            // dd($Po);
            $Po->qty = json_decode($Po->qty);
        $data = $this->mastersDatas();
        $editPurchaseOrder = "";
        return view("printedpurchaseorder.create", compact("data","editPurchaseOrder","Po"));
    }
    ...
    public function store(WovenRequest $request)
    {
        ...
        return redirect()
                ->route("printed.purchaseorder.index")
    ...
    public function edit($woven)
    {   
        $data           = $this->mastersDatas();
            
     $Po = PrintedPurchaseOrder::where(['id'=>$woven])->with('designCard')->first();
            
        $editPurchaseOrder = $Po->designCard;
            // dd($Po);
            $Po->qty = json_decode($Po->qty);
             $Po->qty_title = json_decode($Po->qty_title);
    ...
    public function createpo($woven)
    {   
        $data           = $this->mastersDatas();
        $editPurchaseOrder = DesignCard::where('id',$woven)->first();
            
            $Po = PrintedPurchaseOrder::Create(['design_id'=>$editPurchaseOrder->id]);
            // dd($Po);
            $Po->qty = json_decode($Po->qty);
        ...
        return  redirect()->route('printed.purchaseorder.edit',$Po->id);
        
    }
    

    public function update(PoRequest $request, $woven)
    {
        try {
            $mulitpleDesignFile = [];
            ...
            $po = $validatedFields;
           // dd($po);
            $po['qty'] = json_encode($po['qty']);
            $po['qty_title'] = json_encode($po['qty_title']);
            
            $po = PrintedPurchaseOrder::where('id',$woven)->update($po);
        
            return redirect()
        ->route("printed.purchaseorder.index")
        ->with("warning", "Purchase Order updated successfully.");
        } catch (\Exception $exception) {
            DB::rollBack();
            return redirect()
                ->back()
    ...
    public function show($woven)
    {
        ...
        $purchaseOrders = PrintedPurchaseOrder::with('designCard')->findOrFail($woven);
        
        $viewDesignCard  = DesignCard::with(['customerDetail','designerDetail','salesRepDetail','warpDetail','foldMasterDetail','categoryMasterDetail'])->findOrFail($purchaseOrders->design_id);
        ...
        return view("printedpurchaseorder.show", compact("purchaseOrders","viewDesignCard","weavers","Po"));
    }
    ...
    public function destroy(DesignCard $woven)
    {
        ...
        return redirect()
        ->route("printed.purchaseorder.index")
        ->with("danger", "Design card deleted successfully.");
    }
